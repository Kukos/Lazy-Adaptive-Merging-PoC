...
void db_stat_start_query(void)
{
    ...
    LOG("Start new query\n");
    db_stat_reset_query();
    db_current_query.memory_usage = db_total.memory_usage;
}
...
void db_stat_finish_query(void)
{
    ...
    {
        db_minimum_snapshot.read_time = db_current_query.read_time;
        db_minimum_snapshot.write_time = db_current_query.write_time;
        db_minimum_snapshot.erase_time = db_current_query.erase_time;
        db_minimum_snapshot.memory_usage = db_current_query.memory_usage;
    }
    else /* now min = min(min, current) */
    {
        ...
        db_minimum_snapshot.memory_usage = MIN(db_minimum_snapshot.memory_usage, db_current_query.memory_usage);
    }

    /* update total */
    db_total.read_time += db_current_query.read_time;
    db_total.write_time += db_current_query.write_time;
    db_total.erase_time += db_current_query.erase_time;

    /* current memory usage is in total usage */
    db_total.memory_usage = db_current_query.memory_usage;

    /* update maximum snapshot */
    if (db_stat_get_max_time() < db_stat_get_current_time())
    {
        db_maximum_snapshot.read_time = db_current_query.read_time;
        db_maximum_snapshot.write_time = db_current_query.write_time;
        db_maximum_snapshot.erase_time = db_current_query.erase_time;
    }
    db_maximum_snapshot.memory_usage = MAX(db_maximum_snapshot.memory_usage, db_current_query.memory_usage);
}
